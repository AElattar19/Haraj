...
class SiteMapController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data = collect([
            [
                'name'             => 'services',
                'index_route_name' => 'services.index',
                'show_route_name'  => 'services.show',
                'data'             => Store::query(),
            ],
        ]);
    }
    ...
    public function viewer(Request $request, $name, $page)
    {
        ...
        foreach ($items as $item) {
            $url = '<url><loc>'.str_replace('&', '-', route($route, $item)).'</loc><priority>1.0</priority><lastmod>'.gmdate(DateTime::W3C,
                    strtotime($item->updated_at)).'</lastmod></url>';
            $urls[] = $url;
        }
        $urls = implode('', $urls);

        return response('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">'.$urls.'</urlset>',
            200, [
    ...
    public function generator($items = [])
    {
        ...
        $urls[] = '<sitemap><loc>'.env('APP_URL').'/sitemaps/links'.'</loc></sitemap>';
        $urls = implode('', $urls);
